 ...
 public class CartViewAction
 {
   ...
   public void add_goods_cart(HttpServletRequest request, HttpServletResponse response, String id, String count, String price, String gsp, String buy_type)
   {
     ...
     if (add) {
       ...
       sc33.setTotal_price(BigDecimal.valueOf(
           ...
           for (StoreCart sc1 : cart) { //sc = (StoreCart)gc.next();
             for (int i = 0; i < ((StoreCart)sc1).getGcs().size(); i++) {
               ...
               if (art.getId().toString().equals(cart_id)) {
                 ((StoreCart)sc1).setTotal_price(BigDecimal.valueOf(CommUtil.add(
                   ((StoreCart)sc1).getTotal_price(), 
                   Double.valueOf((CommUtil.null2Int(count) - art
                   .getCount()) * 
                   CommUtil.null2Double(art
                   .getPrice())))));
           ...
           for (int i = 0; i < scart.getGcs().size(); i++) {
             ...
             if (gcart.getId().toString().equals(cart_id)) {
               scart.setTotal_price(BigDecimal.valueOf(
                 CommUtil.add(scart.getTotal_price(), 
                 Double.valueOf((CommUtil.null2Int(count) - gcart
                 .getCount()) * 
     ...
     for (StoreCart ssscart : cart ) { 
       ...
       if (ssscart.getStore().getId().equals(CommUtil.null2Long(store_id))) {
         ((Map)map).put("sc_total_price", 
           Float.valueOf(CommUtil.null2Float(ssscart.getTotal_price())));
       }
     }
       ...
       if (CommUtil.null2String(cart_session1).equals(cart_session)) {
         ...
         if (cookies != null)
         {
          // Cookie[] arrayOfCookie1;
           //GoodsCart localGoodsCart1 = (arrayOfCookie1 = cookies).length; 
